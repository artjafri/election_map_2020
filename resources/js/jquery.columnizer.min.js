    ...
    var DATA_ORIGINAL_DOM_KEY = 'columnizer-original-dom'; $.fn.columnize = function (options) {
        this.each(function () { var $el = $(this); $el.data(DATA_ORIGINAL_DOM_KEY, $el.clone(true, true)); }); this.cols = []; this.offset = 0; this.before = []; this.lastOther = 0; this.prevMax = 0; this.debug = 0; this.setColumnStart = null; this.elipsisText = ''; var defaults = { width: 400, columns: false, buildOnce: false, overflow: false, doneFunc: function () { }, target: false, ignoreImageLoading: true, columnFloat: "left", lastNeverTallest: false, accuracy: false, precise: false, manualBreaks: false, disableSingle: false, cssClassPrefix: "", elipsisText: '...', debug: 0 }; options = $.extend(defaults, options); var tables = new Array(); $('table').each(function () {
            if (!$(this).hasClass('tableSaved')) {
                ...
                tables.push({ tableID: 'tableID-' + tables.length, thead: $(this).find('thead:first').clone(), tfoot: $(this).find('tfoot:first').clone() });
            }
        }); function fixTables($pullOutHere) {
            for (i = 0; i < tables.length; i++) {
                if ($pullOutHere.is("table")) {
                    if ($pullOutHere.children('tfoot').length == 0 && $pullOutHere.hasClass(tables[i].tableID)) { $(tables[i].tfoot).clone().prependTo($pullOutHere); }
                ...
                $pullOutHere.find('table .' + tables[i].tableID).each(function () {
                    if ($(this).children('tfoot').length == 0) { $(tables[i].tfoot).clone().prependTo(this); }
        ...
        return this.each(function () {
            ...
            function split($putInHere, $pullOutHere, $parentColumn, targetHeight) {
                if ($putInHere.contents(":last").find(prefixTheClassName("columnbreak", true)).length) { fixTables($pullOutHere); return; }
